...
function Connection(config) {
  ...
  this._config = {
    ...
    connTimeout: config.connTimeout || 20000,
  ...
  this._stream = config.stream || undefined;
  this._tlsEngine = config.tlsEngine || undefined;
  this._tagcount = 0;
...
// Re-call login function on TLS secured connection
...
Connection.prototype.connect = function() {
  ...
  function onconnect() {
    ...
    self.debug && self.debug('[socket] (connected to server)');
  ...
  this._onError = function(err) {
    ...
    self.debug && self.debug('[socket] (error) ' + err);
    err.source = 'socket';
    self.emit('error', err);
  };
  ...
  this._onSocketTimeout = function() {
    ...
    self.debug && self.debug('[socket] (socket timedout)');
  ...
  socket.once('close', function(had_err) {
    ...
    clearTimeout(self._tmrKeepalive);
    self.state = 'disconnected';
    self.debug && self.debug('[socket] (close socket)');
  ...
  socket.once('end', function() {
    ...
    clearTimeout(self._tmrKeepalive);
    self.state = 'disconnected';
    self.debug && self.debug('[socket] (end socket)');
    self.emit('end');
  });

  // Give main socket to parser's constructor
  ...
  parser.on('continue', function(info) {
    ...
    if (type === 'IDLE') {
      if (self._queue.length
          ...
          && self._sock.writable
          && !self._idle.enabled) {
        self.debug && self.debug('=> [imap] (continue parser) DONE');
        if (self._tlsEngine.isConnected){
          self._tlsEngine.prepare(forge.util.encodeUtf8('DONE' + CRLF))
        } else {
      ...
      self.debug && self.debug('[imap] => ' + inspect(CRLF));
      ...
      self.debug && self.debug('[imap] => ' + inspect(line));
...
Connection.prototype.end = function() {
  ...
  this._enqueue('LOGOUT', function() {
    self.debug && self.debug('[imap] => (received logout confirm, end socket)');
    self._queue = [];
    self._curReq = undefined;
    self._sock.end();
  });
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === 'bye'){
    self.debug && self.debug('[imap] (bye command)');
    // this._sock.end();
          ...
          if (!self.serverSupports('IDLE')
            ...
            if (self._idle.started === undefined) {
              ...
              if (timeDiff >= idleWait) {
                self._idle.enabled = false;
                self.debug && self.debug('[imap] => (keep alive timer) DONE');
                if (self._tlsEngine.isConnected){
                  self._tlsEngine.prepare(forge.util.encodeUtf8('DONE' + CRLF))
                } else {
                  self._sock.write('DONE' + CRLF);
...
Connection.prototype._login = function() {
  ...
  this.debug && this.debug('[imap] (fetching capabilities)')
...
Connection.prototype._starttls = function() {
  ...
  this.debug && this.debug('[imap] (upgrade to tls session)')
  this._enqueue('STARTTLS', function(err) {
    ...
    */
    ...
    this.debug && this.debug('[imap] (call tls and redirect streams)')

    // Redirect readable/writable stream to library's parser
    self._parser.setStream(this._stream)

    // Handshake with newly created connection
    self._tlsEngine.handshake()
...
Connection.prototype._processQueue = function() {
  ...
  this.debug && this.debug('[imap] => ' + inspect(out));

  // Write to socket or TLS engine
  if (this._tlsEngine && this._tlsEngine.isConnected){
    this._tlsEngine.prepare(forge.util.encodeUtf8(out + CRLF))
  } else {
...
Connection.prototype._sockWriteAppendData = function(appendData)
{
  ...
  this.debug && this.debug('[imap] => ' + inspect(val));
  // Write to socket or TLS engine
  if (this._tlsEngine && this._tlsEngine.isConnected){
    this._tlsEngine.prepare(forge.util.encodeUtf8(val))
    this._tlsEngine.prepare(forge.util.encodeUtf8(CRLF))
  } else {
...
Connection.prototype._enqueue = function(fullcmd, promote, cb) {
  ...
  if (!this._curReq
    ...
    if (this._idle.started > 0) {
      ...
      this.debug && this.debug('[imap] => (enqueue) DONE');
      // Write to socket or TLS engine
      if (this._tlsEngine && this._tlsEngine.isConnected){
        this._tlsEngine.prepare(forge.util.encodeUtf8('DONE' + CRLF))
      } else {
